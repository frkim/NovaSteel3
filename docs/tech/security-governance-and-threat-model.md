...
10. **No agent both retrieves untrusted content and holds a calculation tool.** The two retrieval agents (`novasteel-procedure-agent`, `novasteel-web-search-agent`) declare retrieval tools only, so a prompt injected into a retrieved procedure or web result reaches a model that has no dispatch, RUL, carbon or yield tool to call and cannot acquire one at runtime. This was a platform-level control until ADR-020 collapsed the roster into one Foundry project; it is now enforced in the manifest and asserted in CI (`tests/knowledge/test_agent_manifest.py` fails the build if either agent gains a function tool), and at the API boundary, where `POST /v1/copilot/agent` admits only agents that declare a calculation tool and returns `403` for the others (`tests/backend/test_agent_routes.py`). **This is a weaker control than the project separation it replaced** — the separation was structural, this is asserted — and it is recorded here rather than in the ADR alone because a change to those tests is a change to a security control and must be reviewed as one.
