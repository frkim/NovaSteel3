...
| Loading (KPI band) | `STATE-LOAD-KPI` | Every tile shows an em-dash with neutral status and no trend, target, sparkline or freshness marker | KPI bands are computed outside the panel `StateBoundary`, so an empty result set would otherwise render a derived `0` — which reads as a measured, healthy value. `KpiBand pending` swaps the tiles for placeholders until data arrives, and stays on if the fetch fails | `aria-busy="true"` on the band; each tile keeps its label and tooltip |
