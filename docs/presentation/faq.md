...
**Q88. Where does this run, and why two Foundry projects?**
The energy advisor is hosted in **Azure AI Foundry Agent Service**, in the *operations* project (`proj-novasteel-ops-<env>`), separate from the *knowledge* project that holds the agents which read untrusted content (ADR-019). That split is a trust boundary, not a namespace: an agent can only call tools declared on its own definition in its own project, so a prompt injected into a retrieved procedure has no path to the dispatch optimizer — the agent that read it has no such tool and cannot acquire one at runtime. The roster lives in one reviewable manifest (`agent_manifest.py`) and is applied at release time by `agent_reconciler.py` rather than created lazily on a user's request, because lazy creation is how the estate ended up with two defined agents and an empty Agent Service.
