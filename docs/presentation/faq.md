...
Three things the screen genuinely cannot do. **Natural language in**: a planner says "can we shift tonight's load without moving anything more than 90 minutes?" instead of finding the right sliders — the constraint is extracted and passed to the solver. **Explanation out**: the solver returns a schedule and a cost delta, and the agent turns that into the sentence a planner needs, naming the solver strategy, the constraint violations and the fact that it is a proposal. **Composition**: the dispatch agent and the lining-RUL agent can be reasoned about together — "if I delay this batch, what does it do to the hearth?" — which no single screen answers today. What the agent explicitly does *not* add is any new decision authority.

**Q85. What stops the agent from inventing a schedule or a saving figure?**
Structure, not prompt wording. The model has exactly four callable tools — `read_energy_context`, `forecast_demand`, `simulate_schedule`, `propose_recommendation` — and they are the same four names already allow-listed in `ENERGY_AGENT_TOOLS`; every call is routed through `ToolRegistry`, which refuses any name not on the list. `commit_schedule` and `approve_recommendation` sit in `FORBIDDEN_TOOL_NAMES` and have no port method behind them at all, so there is nothing to call even if the model tried. The only path to a number is `simulate_schedule`, which runs the MILP. If the optimizer is unreachable the tool returns a structured error and the agent **declines** rather than estimating — a decline is a correct answer, an invented percentage is not. The tool schemas do not even require a site code, because a model asked for one will confidently invent a plausible-looking plant identifier; the site is resolved by the service that actually knows the deployment's plants.

**Q86. So the agent can change tonight's production schedule?**
No. It can *propose*; a human disposes. `simulate_schedule` produces a recommendation in `PENDING_APPROVAL` and `propose_recommendation` is deliberately a read-back with no write — if it could change a status, the propose/approve boundary would exist only in the prose. Committing a schedule is a separate, policy-gated route that the agent has no tool for. Every agent-initiated solve is written to the same audit trail as a planner-initiated one, attributed to the agent identity (`energy-dispatch-agent`) rather than to the signed-in user, so an auditor can always tell which solves a human asked for and which the agent ran.

**Q87. What happens when the solver is not available in the deployed container?**
The optimizer falls back to a deterministic heuristic and *says so*: the result carries `solver: "DETERMINISTIC_HEURISTIC"` instead of `"MILP_CBC"`, and the agent's answer changes with it — it reports a schedule that is "feasible and repeatable, but not proven optimal" rather than "the proven cheapest schedule". The claim of optimality is only ever made when a MILP actually proved it. The same honesty applies one level up: if the Foundry-hosted agent is unreachable, the deterministic local agent answers from the *same* solver output, so a Foundry outage degrades the prose, not the numbers.

**Q88. Where does this run, and what happens if Foundry is not deployed?**
The agent is hosted in **Azure AI Foundry Agent Service** as a versioned agent with the four function tools declared on it; the tool loop runs on our side, so the tools execute inside our BFF with our identity and our audit trail rather than in a hosted sandbox. Foundry agents are data-plane objects, not ARM resources, so the Bicep provisions the project and the connections and the agent version is created at runtime. If `FOUNDRY_PROJECT_ENDPOINT` is not set, or the project is not deployed, `create_energy_dispatch_agent` returns the deterministic local agent and logs why — the same graceful-degradation pattern every other Azure integration in this codebase follows. The demo therefore works with no Azure subscription at all.

**Q89. How does a planner reach it — is this Microsoft 365 Copilot?**
Not today. It is wired into the platform's own Copilot side panel: the panel classifies each question and routes *dispatch requests* to the energy agent, leaving definitions, lookups and glossary questions with the ordinary chat agent. The classifier is deliberately conservative — it needs both an action word and an energy word ("optimize the energy schedule"), or an action word while the planner is already standing on the energy-optimization screen. "What does CO₂ intensity mean?" contains an energy word and no action word, so it stays with the chat agent, which is the right outcome because the optimizer has nothing to say about a definition. Missing a dispatch question costs a re-phrase; routing a definition question into the solver wastes a solve and answers the wrong question. If the routed answer comes back ungrounded, the panel silently falls through to the normal chat agent, so the feature can only add answers, never remove them. Exposing the same agent through M365 Copilot is a later step and a different governance conversation — it would put plant scheduling data in front of a much broader audience.

**Q90. Why route inside the process rather than calling your own API back over HTTP?**
Because the Copilot route has already authenticated the caller and applied `require_reader` before anything reaches the agent. Re-entering over HTTP would need either a second service-to-service credential — a new secret to protect for no capability gained — or would drop the user's identity from the audit record. Calling `simulate_energy` directly means an agent-initiated solve is written to the audit trail by exactly the same code path as a planner-initiated one, which is precisely the property an auditor wants.
