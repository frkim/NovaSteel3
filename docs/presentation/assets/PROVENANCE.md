...
| Property | Value |
|---|---|
| Source | `app-guide/screenshots/*.png` in this folder — nothing external is added |
| Generator | `tools/presentation/annotate_demo_screenshots.py` (Pillow only, no network access) |
| Licence risk | **None.** Derivative of first-party captures; the red frames and captions are generated by this repository's own script. |

Regenerate with `python tools\presentation\annotate_demo_screenshots.py` after re-capturing a screen
or moving a highlight.
