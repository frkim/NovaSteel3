...
6. **CompanyWebsiteDiagram / ProcessDiagram.** Lower sections include six pieces of zoomable artwork. Three are **full-width diagrams** — the integrated BF/BOF route, the EAF route, and the detailed EAF process — that stretch across the column because their small labels need the space. Three are **figures** rendered in an identical 460 px, 4:3 frame so they line up with each other: a blast furnace cutaway under *The blast furnace route*, and a rolling-stand close-up plus a hot rolling mill line under *Shaping Metals → Rolling in practice*. Clicking any of them opens a lightbox with zoom from 100% to 400% (`apps\analytics-mfe\src\components\screens\CompanyWebsiteSteelKnowledge.tsx:261-380`; `apps\analytics-mfe\src\components\screens\CompanyWebsiteDiagram.tsx:16-60`).
...
| Process context for AI | AI-01, AI-02 | Diagrams and photographs show the equipment later monitored by Furnace Health and Energy Optimization — the blast furnace cutaway names the 1,600 °C hearth that wears the lining, and the rolling mill figures show the hot metal whose reheating drives the energy bill. | No screen API; `/media/*.webp` images rendered by `ProcessDiagram` (`apps\analytics-mfe\src\components\screens\CompanyWebsiteDiagram.tsx:16-60`, `apps\analytics-mfe\src\components\screens\CompanyWebsiteSteelKnowledge.tsx:273-345`). |
...
**Try it yourself.** Open `http://localhost:5266/lu/company-website/steel-knowledge`, click **Making Iron & Steel**, then click the blast furnace cutaway and try the zoom controls. Then open **Shaping Metals** and compare the two rolling mill figures — same frame, one close-up and one plant-scale view.
