...
The shell passes a typed context defined by `contracts\ui\shell-interop.v1.schema.json`: `themeMode`, `locale`, `activePersona`, `site`, an opaque `tokenRef`, `bridgeVersion`, and `navigation` (`contracts\ui\shell-interop.v1.schema.json:7-15`). The React app is mounted through `AnalyticsBridge`, which calls JavaScript `mount` and later `update` when context changes (`apps\portal-shell\Components\AnalyticsBridge.razor:20-39`).
...
| BFF connection pill | Automatic backend-reachability indicator; replaces the former demo/cloud mode toggle. | `BFF…` while checking, then `BFF online` or `BFF offline`; click to re-check. | The portal is always cloud-backed, so there is no mode to choose: the shell probes the BFF on start and states honestly whether it answered, falling back to bundled synthetic fixtures when it did not. | `apps\portal-shell\Layout\MainLayout.razor:68-71`; probe behavior `apps\portal-shell\Services\ShellState.cs:225-266`. |
...
| Settings dialog | Modal for appearance, language, data, and help preferences. | `Settings`, Appearance Light/Dark/System, Locale, read-only BFF base URL, bilingual help checkbox. | Groups durable shell preferences and traps focus for accessibility. | `apps\portal-shell\Components\SettingsDialog.razor:5-75`, focus handling `apps\portal-shell\Components\SettingsDialog.razor:103-160`. |
| Synthetic-data banner | Purple honesty banner, rendered unconditionally. | `Synthetic demo data — not for operational control` across the top and inside the dashboard. | The data is synthetic wherever it is read from, so the guardrail is never gated on a mode; it prevents confusion between demo predictions and operational measurements. | Shell banner `apps\portal-shell\Layout\MainLayout.razor:120-122`; React banner `apps\analytics-mfe\src\components\AnalyticsDashboard.tsx:104-117`; advisory boundary `docs\architecture\solution-architecture.md:24-29`. |
...
| Footer | Always-visible connection and accessibility note. | Left: `BFF http://localhost:8080 · connected` (or `unreachable — using bundled synthetic data`, or `connection not yet checked`); right: `WCAG 2.2 AA target · synthetic evidence only`. | Reminds users of the backend source and whether it is reachable, the accessibility target, and that the evidence is synthetic. | `apps\portal-shell\Layout\MainLayout.razor:162-165`; WCAG target in `docs\ux\dashboard-specification.md:14-16`. |
...
The settings screenshot shows appearance radios, locale selector, the read-only BFF base URL `http://localhost:8080`, and the bilingual help option. These settings map to `ThemeMode`, `Locale`, and `HelpBilingual` in `ShellState` (`apps\portal-shell\Components\SettingsDialog.razor:18-72`).
