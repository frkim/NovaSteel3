...
| Persona selector | Primary role selector. | `Plant Manager` in the top bar. | A person can hold several demo personas; the shell can route to that persona's default area. | `apps\portal-shell\Layout\MainLayout.razor:27-35`, `apps\portal-shell\Layout\MainLayout.razor:204-209`; personas in `docs\personas\personas-and-journeys.md:44-53`. |
...
| Left navigation rail | Persistent section navigation. | Groups: DAILY OPERATIONS, INSIGHT & GOVERNANCE, PLATFORM & REFERENCE. | Lets beginners find role surfaces while preserving cross-role navigation. | `apps\portal-shell\Layout\MainLayout.razor:125-145`; group definitions `apps\portal-shell\Services\ShellState.cs:40-58`. |
