...
|---|---|---|
| Blazor shell | Chrome, route grammar `/{site}/{section}/{subView}`, site/persona selectors, locale, theme, demo mode, account menu, capacity panel. | The architecture keeps a C# shell while React handles dense dashboards (`docs\architecture\solution-architecture.md:40-50`; `apps\portal-shell\Pages\AnalyticsHost.razor:1-13`). |
| Bridge / contract | `themeMode`, `locale`, `activePersona`, `site`, `demoMode`, `tokenRef`, `bffBaseUrl`, `permittedActions`, `navigation`, bridge version `1.0`. | The shell and MFE exchange a versioned, typed shape instead of guessing (`contracts\ui\shell-interop.v1.schema.json:1-83`; `apps\analytics-mfe\src\types.ts:9-35`). |
