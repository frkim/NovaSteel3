...
| Sélecteur de persona | Rôle principal. | `Plant Manager`. | Un utilisateur peut avoir plusieurs personas; le shell route vers la surface par défaut. | `apps\portal-shell\Layout\MainLayout.razor:27-35`, `apps\portal-shell\Layout\MainLayout.razor:204-209`; `docs\personas\personas-and-journeys.md:44-53`. |
...
| Rail gauche | Navigation permanente. | DAILY OPERATIONS, INSIGHT & GOVERNANCE, PLATFORM & REFERENCE. | Aide les débutants à trouver les surfaces par rôle. | `apps\portal-shell\Layout\MainLayout.razor:125-145`; `apps\portal-shell\Services\ShellState.cs:40-58`. |
