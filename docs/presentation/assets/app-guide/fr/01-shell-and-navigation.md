...
Le shell transmet un contexte typé défini par `contracts\ui\shell-interop.v1.schema.json` : `themeMode`, `locale`, `activePersona`, `site`, `tokenRef` opaque, `bridgeVersion` et `navigation` (`contracts\ui\shell-interop.v1.schema.json:7-15`). React est monté par `AnalyticsBridge`, qui appelle `mount` puis `update` si le contexte change (`apps\portal-shell\Components\AnalyticsBridge.razor:20-39`).
...
| Pastille de connexion BFF | Indicateur automatique de joignabilité du backend ; remplace l'ancien sélecteur de mode démo/cloud. | `BFF…` pendant la vérification, puis `BFF online` ou `BFF offline` ; cliquer pour revérifier. | Le portail est toujours adossé au cloud : aucun mode à choisir. Le shell sonde le BFF au démarrage et indique honnêtement s'il a répondu, en repliant sur les fixtures synthétiques embarquées sinon. | `apps\portal-shell\Layout\MainLayout.razor:68-71` ; comportement de la sonde `apps\portal-shell\Services\ShellState.cs:225-269`. |
...
| Settings | Boîte de dialogue de préférences. | Appearance, Locale, URL BFF en lecture seule, aide bilingue. | Regroupe préférences et piège le focus. | `apps\portal-shell\Components\SettingsDialog.razor:5-75`, `apps\portal-shell\Components\SettingsDialog.razor:103-160`. |
| Bandeau synthétique | Avertissement violet, affiché en permanence. | `Synthetic demo data — not for operational control`. | Les données sont synthétiques quelle que soit leur source ; le garde-fou n'est donc jamais conditionné à un mode, et évite de confondre démonstration, prédiction et mesure. | `apps\portal-shell\Layout\MainLayout.razor:120-122`; `apps\analytics-mfe\src\components\AnalyticsDashboard.tsx:104-117`; `docs\architecture\solution-architecture.md:24-29`. |
...
| Pied de page | Note de connexion et d'accessibilité. | `BFF http://localhost:8080 · connected` (ou `unreachable — using bundled synthetic data`, ou `connection not yet checked`) et `WCAG 2.2 AA target · synthetic evidence only`. | Rappelle la source backend et sa joignabilité, la cible WCAG et le caractère synthétique de la preuve. | `apps\portal-shell\Layout\MainLayout.razor:162-165`; `docs\ux\dashboard-specification.md:14-16`. |
...
La capture `Settings` montre les radios Appearance, la locale, l'URL BFF en lecture seule `http://localhost:8080` et l'option d'aide bilingue. Ces choix correspondent à `ThemeMode`, `Locale` et `HelpBilingual` (`apps\portal-shell\Components\SettingsDialog.razor:18-72`).
