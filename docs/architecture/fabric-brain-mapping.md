...
| 2.8 | Full CI/CD round-trip | `.github/workflows/cd-fabric-items.yml` + `cicd-fabric-agent` + `fabric-cli-agent` | Workflow completes on protected feed; OIDC token resolves correctly against the federated credential registered in `.github/CONFIGURATION.md` (Repository-level variables section) |
