...
| `/v1/copilot/agent` | POST | Reader at the route; per-tool role and site inside | Reaches the tool-calling operations agents (ADR-019): four specialists — energy, carbon, quality, maintenance — and one orchestrator for questions spanning them, selected by a deterministic keyword router unless the caller names an agent. The tools re-apply the caller's roles and plant scope, so a reader reaching this route is still refused by any tool they are not entitled to invoke, and routing therefore grants nothing. Returns proposals with `modelVersion` and `auditRef`, never a commitment. |
