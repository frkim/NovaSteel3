...
| Demo capacity control | UX makes capacity controls interactive; Fabric research warns a paused capacity is unavailable. | Capacity control is **always simulated in the demo deployment**. `bff-api` selects `LocalCapacityAdapter` whenever the service runs in its demo configuration, and reports this to the UI as `demoModeSimulated` so the Platform Ops panel states plainly that no ARM call was made. A real resume is possible only for an authorized operator holding `Platform.Capacity.Manage` against a non-demo configuration, and only for the allow-listed demo capacity. It never controls production capacity from the presentation path. |
...
| `/v1/copilot/agent` | POST | Reader at the route; per-tool role and site inside | Reaches the tool-calling operations agents (ADR-019, ADR-020): four specialists — energy, carbon, quality, maintenance — and one orchestrator for questions spanning them, selected by a deterministic keyword router unless the caller names an agent. Admission is by definition — only an agent declaring a calculation tool is reachable here, so naming a retrieval agent is refused. The tools re-apply the caller's roles and plant scope, so a reader reaching this route is still refused by any tool they are not entitled to invoke, and routing therefore grants nothing. Returns proposals with `modelVersion` and `auditRef`, never a commitment. |
...
**Status:** Accepted; amended by ADR-017 (the user-facing mode toggle is removed — offline replay is now a build-time and fallback behaviour, not a runtime choice).  
...
**Status:** Accepted; scoped by ADR-019 as amended by ADR-020 (the *chat* surface still receives no tools; the tool-calling agents are separate agent definitions, no longer a separate project). A narrowing amendment is proposed in [copilot-fabric-data-agent.md](copilot-fabric-data-agent.md) — the chat would gain the Fabric data agent as one additional *grounding source*, still with no tools of its own.  
...
### ADR-015 — The Help Assistant resolves topics from the DOM, not from screen edits

**Status:** Accepted.  
**Context:** The application has on the order of two dozen screens composed of shared primitives. A learnability feature that required every screen to declare its own help content would mean touching every screen file, would rot immediately as screens change, and would be impossible to keep consistent across five locales.  
**Decision:** Resolve help topics by inspecting the DOM at click time — structural shape first, then declared `data-help` attributes on shared primitives, then generic element shape — instead of maintaining a per-screen help registry. Content lives in one catalog per locale plus satellite files, and a test enforces byte-level key and optional-field parity across all locales.  
**Alternatives considered:** A per-screen help registry (rejected: touches every screen, rots as screens change, duplicates content across locales). Reusing the existing `metric.tooltip` strings as help text (rejected: expert-facing register written for operators, wrong audience for a newcomer — surfaced instead as `data-help-detail`, a secondary italicised line). A separate documentation site (rejected: breaks the user's flow, and the requirement was explicitly in-context explanation during a live demonstration).  
**Consequences:** The entire application became explainable by annotating three shared primitives (`KpiCard`, `DataTable`, `ChartContainer`) plus two screen files with specific content needs; new screens built from those primitives are explainable for free. Content lives in one place per locale and is parity-tested. The negative trade-off: help quality for a bespoke, one-off component degrades to a generic fallback unless someone adds `data-help`; resolution depends on DOM structure, so a refactor that changes element shape (e.g. replacing an `<article>` with a `<div>`) can silently change the help text shown. Mitigations: the `data-help-surface` escape hatch prevents the assistant from interfering with its own chrome; the resolution precedence order is covered by unit tests (`help.test.tsx`) that pin the structural → declared → loose priority and will fail if a DOM refactor changes the outcome.

### ADR-016 — Azure Event Hubs, not Azure IoT Hub, is the telemetry ingress buffer

**Status:** Accepted.  
**Context:** The ingress path is a per-plant OT gateway in the industrial DMZ that terminates OPC UA, MQTT, or historian exports and emits a versioned envelope northbound into `eh-telemetry-<plant>`, from which `ingest-relay` publishes to the Fabric Eventstream Custom Endpoint with a workload identity (§4.1, ADR-005). Because the sources are industrial assets, Azure IoT Hub is the obvious alternative and is a fair challenge to the design.  
**Decision:** Keep Azure Event Hubs. Do not adopt IoT Hub.  
**Rationale:**
1. **The value of IoT Hub is a control plane we have promised not to build.** Its differentiators over Event Hubs are cloud-to-device: device provisioning (DPS), twins, desired properties, direct methods, and jobs. Every one of those is an inbound path into the plant. ADR-007 and the O1/C-04/AI-05 guardrails make this platform advisory-only, with no write path to OT. Choosing IoT Hub would mean acquiring an inbound control plane and then having to disable it, restrict it, and prove in the threat model that it stays disabled — a net increase in attack surface for capability we must not use.
2. **It would weaken the no-standing-secret posture.** The Event Hubs namespace runs with `disableLocalAuth: true`, public network access disabled, a private endpoint, and per-plant `mi-ns-otgw-<plant>` identities granted Azure Event Hubs Data Sender **on a single hub**, not the namespace (`infra/bicep/modules/eventhubs.bicep`). IoT Hub device authentication is per-device symmetric keys or X.509 certificates; Entra ID covers its service and management plane, not device auth. Fabric's IoT Hub source for Eventstream is likewise a shared-access-policy connection, which is the exact key-based pattern ADR-005 exists to avoid.
3. **The senders are four gateways, not a device fleet.** IoT Hub's per-device identity and lifecycle value scales with device count. NovaSteel has one gateway per plant; device-level identity is carried in the event envelope (`source_id`, `asset_id`) and modelled in the Device Operations registry (17 devices / 91 sensors). We would pay for a device registry we already have at the application layer.
4. **Cost and footprint.** The demo posture is deliberately minimal — one Event Hubs Standard throughput unit, a small Fabric capacity, and a nightly pause. IoT Hub adds a second billable ingestion service for no consumed capability.
5. **It is not the strategic industrial path anyway.** As of 2026 IoT Hub is fully supported and not retired, but Microsoft's direction for OT and industrial scenarios is Azure IoT Operations (Arc-enabled, MQTT broker, OPC UA connector, data flows into Fabric). Replacing Event Hubs with IoT Hub would move toward the older device-centric service, not the strategic one.

**Alternatives considered:** *IoT Hub as ingress* (rejected, above). *IoT Hub with C2D features administratively disabled* (rejected: the capability still exists on the resource and must be continuously proven off; Event Hubs simply cannot do it). *Azure IoT Operations at the edge* (deferred, not rejected — it is the right answer the day the OT edge, gateway lifecycle, or per-sensor cloud identity comes into scope, and it publishes to Fabric natively). *Fabric's native Event Hubs source without a relay* (already rejected in ADR-005: SAS key).  
**Consequences:** We keep an ingress that can only receive. Per-device provisioning, OTA configuration, and remote command are explicitly out of platform scope and would require a new ADR, a threat-model update, and — because closed-loop control of industrial machinery changes the EU AI Act risk classification — a fresh conformity assessment. The trigger for revisiting this decision is a business requirement for cloud-initiated action on plant equipment, or an estate where gateways are no longer able to aggregate upstream devices; the migration target in that case is Azure IoT Operations, not IoT Hub.

### ADR-017 — The portal has one data path, and honesty is unconditional

**Status:** Accepted. Supersedes the user-facing half of ADR-008.
**Context:** The shell used to carry a `DEMO`/`CLOUD` toggle in the top bar. It conflated three unrelated things under one word. *Demo* meant "read the committed fixture pack rather than the backend"; but the analytics microfrontend already fell back to those same fixtures automatically whenever a BFF call failed, and `VITE_FIXTURES_ONLY` already forced that at build time. Meanwhile `demoModeSimulated` means "the Fabric capacity adapter is simulated, not real ARM", and the BFF's own `demoMode` config flag means "this deployment serves the synthetic namespace". Three concepts, one label. Worse, the mandatory "Synthetic demo data — not for operational control" banner was rendered **only in DEMO mode**, so the one control that exists to prevent a viewer mistaking this platform for a live plant could be switched off by the person presenting it.

**Decision:** Remove the user-facing mode choice. The portal is always backed by the BFF. On start, the shell probes the unauthenticated `GET /v1/meta` bootstrap route and surfaces the result as a connection indicator (probing / online / offline) that can be re-checked on demand; a failed probe raises a warning toast, a successful automatic probe stays silent. The synthetic-data banner renders **unconditionally**. The microfrontend's fixture fallback is retained untouched as a resilience rung, not as a mode. `demoMode` is removed from the shell↔microfrontend bridge contract (`contracts/ui/shell-interop.v1.schema.json`); `demoModeSimulated` and the BFF's server-side `demoMode` are unaffected.

**Alternatives considered:** *Keep the toggle but force the banner on in both modes* (rejected: it leaves the three-way naming collision in place and still lets a presenter claim a cloud path that is not there). *Keep the toggle for offline rehearsal* (rejected as redundant: `VITE_FIXTURES_ONLY` already produces a genuinely offline build, and the automatic fallback covers an unplanned outage better than a human remembering to flip a switch mid-demonstration).

**Consequences:** A viewer can no longer be shown a `CLOUD` badge that is not backed by a reachable service, because the indicator reports what the probe actually returned rather than what was selected. Provenance now comes from the backend instead of the UI: `GET /v1/meta` carries `dataSource`, so the screen states whether rows came from the Fabric lakehouse, the committed fixture pack, or a fixture fallback taken because the capacity was paused (ADR-018). The cost is that offline rehearsal is now a build-time decision rather than a runtime one; that is deliberate, since it makes the offline pack a reproducible artifact instead of a live gesture.

### ADR-018 — Two data streams into Fabric: governed analytical and real-time

**Status:** Accepted.
**Context:** Fabric was declared the core of this solution (ADR-001) and the estate was provisioned — two lakehouses, an Eventhouse, medallion notebooks and pipelines — but it held no NovaSteel data. Every screen was served from a committed 24-hour simulator fixture pack, and the authored `es-ns-telemetry-v1` Eventstream had never been deployed. The claim "Fabric is the core" was therefore architectural rather than demonstrable, which is exactly the kind of gap an examiner is entitled to probe.

**Decision:** Populate Fabric through two deliberately different streams, and let each serve what it is good at.

1. **Static analytical stream.** A deterministic, fixed-seed simulator scenario emits facts at the grain declared in `contracts/data/gold.v2.json` across a multi-month horizon. These land as Delta tables in `lh_novasteelv3_core` through the existing medallion notebooks, keyed by the per-table `idempotencyKey` so a reload is a no-op rather than a duplicate. This is the substrate for KPI trends, the semantic model, and the numbers quoted in the defence. It is *static* in the sense that it changes only when regenerated — which is a feature, because a demonstration that reproduces the same numbers on demand is worth more than one that drifts. Alongside these, the same lakehouse holds nine **operational envelope tables** plus a `manifest` table carrying the committed demo pack at the application grain — that is what the BFF reads, because the BFF consumes whole event envelopes rather than star-schema rows. Two grains, one lakehouse, different consumers; loading only the gold tables would leave the BFF finding nothing and falling back to fixtures silently and forever.
2. **Dynamic real-time stream.** The simulator's `publish` command sends the versioned envelope to the deployed Eventstream Custom Endpoint with an Entra identity (ADR-005). A routing operator fans out on `schema_name` into the Eventhouse hot tables (`telemetry_hot`, `alarm_hot`, `gateway_health_hot`, `model_inference_hot`, `ingest_quarantine_hot`) and, in parallel, into the immutable `bronze_event_envelope` Delta table. This is the substrate for live signal, freshness and alerting.

The BFF reads the analytical stream through a narrow `FabricQueryClient` port against the lakehouse SQL analytics endpoint, authenticating with a managed identity. Selection is configuration (`BFF_DATA_SOURCE`), defaulting to the fixture pack.

**Crucially, Fabric is a preferred source and never a hard dependency.** The F2 capacity is deliberately paused outside demonstration windows for cost control (§ operations), and a paused capacity answers `CapacityNotActive`. That is treated as a soft failure: the BFF falls back to the committed fixture pack, and `GET /v1/meta` reports `dataSource` as `fabric-lakehouse:<lakehouse>`, `simulator-fixture:<pack>`, or `fabric-fallback:simulator-fixture:<pack>` accordingly. The fallback is visible, not silent.
...
**Status:** Superseded by ADR-020. Retained because the boundary it describes still exists — ADR-020 moves it from the platform to the manifest, and the reasoning below is what ADR-020 had to replace rather than discard. Scopes ADR-011; qualifies ADR-012; implements the "restricted simulation tools" half of ADR-006.
...
**Amendment (roster expansion and routing).** The operations project now holds **four specialists and one orchestrator**: the original energy and maintenance advisors, plus a carbon advisor over `carbon_footprint_summary` (Scope 1/2 and ETS exposure) and a quality advisor over `quality_yield_what_if` (bounded first-pass-yield what-if), and `novasteel-operations-orchestrator` holding all four tools. The knowledge project is unchanged and still holds zero function tools, so the boundary above is untouched.

Two constraints shaped the amendment. *One specialist, one calculation:* an agent that accumulates tools stops being legible from its own definition, so each specialist owns exactly one and the orchestrator is the single deliberate exception — it exists for questions that span concerns, and answering "does the cheaper schedule raise our CO2" across two agents would leave the operator to combine two results themselves, which is precisely the unsourced arithmetic ADR-006 forbids. Its instructions therefore require it to name a trade-off and stop, never to weight cost against carbon against asset life; that weighting is the planner's decision.

*Routing is deterministic, not a supervisor model.* `agent_router.py` scores a question against each specialist's declared keywords and selects that specialist on exactly one domain match, or the orchestrator on zero or several; `POST /v1/copilot/agent` uses it when `agent` is omitted or `"auto"`, and returns the decision and its evidence. A supervisor model was rejected on the same grounds as the rest of this ADR: it would make the *choice of calculation* an inference, it is unreviewable (nothing diffs, and a routing regression looks like a good day), and it is a prompt-injection surface, because text carried into the turn could steer which tools become reachable. Routing selects an agent and grants nothing — every tool body still re-applies the caller's roles and plant scope, so a mis-route costs a worse answer, never a wider one. The cost accepted is that keyword routing degrades on vocabulary the manifest does not know; that failure sends the question to the orchestrator, which is the safe direction.

### ADR-020 — One Foundry project; the read/call boundary moves into the manifest

**Status:** Accepted. Supersedes ADR-019 (which is retained above). Does not alter ADR-006, ADR-007, ADR-011 or ADR-012.

**Context:** ADR-019 split the roster across `proj-novasteel-<env>` and `proj-novasteel-ops-<env>` so that the platform itself made a retrieval agent structurally incapable of reaching a calculation tool. Operating it exposed costs that were not visible when the decision was taken.

The estate carries a second project, a second project-scoped capability host that must be serialised behind the first because capability hosts are immutable and concurrent creation on one account races, a second set of Search/Cosmos/Storage connections — including a Search connection the operations project never queries, provisioned only because the capability-host contract demands one — and a second set of RBAC assignments. Every service that talks to agents needs a second endpoint variable, and every environment that runs agents needs both projects deployed or the roster silently half-reconciles. The split also fixed a topology: a Foundry Connected Agent handoff was already deferred in ADR-019 precisely because it would cross the project line.

Against that, the boundary was doing less work than its cost implied. It is not the only control, and on inspection it is not the load-bearing one: the tool *bodies* live in the BFF and re-apply `require_any_role` and `require_site` against the original request scope, the tool registry is deny-by-default, results are marked as proposals rather than decisions, and Prompt Shields runs on the retrieval path. The project split protected against exactly one thing the other controls do not: an agent *definition* that holds both a retrieval tool and a calculation tool.

**Decision:** Collapse the roster into a single Foundry project. In the demo estate that project is named **`novasteelv3`**; in the reference estate it remains `proj-novasteel-<env>`. All seven agents — the procedure and web-search readers, the four specialists and the orchestrator — are deployed there and reconciled through one endpoint (`FOUNDRY_PROJECT_ENDPOINT`).

The read/call boundary is not dropped. It moves from the platform into the manifest, and is enforced by the *definition* rather than by the address:

1. `operations_agents()` returns the specs that declare at least one function tool; `knowledge_agents()` returns the rest. Membership is derived from the definition, so it cannot drift from what an agent can actually do.
2. `POST /v1/copilot/agent` — the only surface that carries tool access — admits an agent only if it is in `operations_agents()`. Naming a knowledge agent there is a 403, not a 404 and not a fallback.
3. The router selects on `domain`, which no reader declares, so a reader is never reachable by routing either.
4. Tests assert the invariant directly against the two readers by name: the procedure and web-search agents must hold no function tool, and no reader may be the orchestrator. That is a build failure, not a review comment.

The `project` field on `AgentSpec`, `agents_for_project()`, `projects()` and the reconciler's per-project loop are all kept. Re-introducing a second project has to remain a data change, not a control-flow change.

**Rationale:**
1. **A structural control that is expensive to keep correct is not obviously safer than an asserted one.** Two projects, two capability hosts and two connection sets are four opportunities for an environment to be half-configured, and a half-configured estate fails toward "agents missing" — which is how ADR-019 was written in the first place, after an empty Agent Service.
2. **The invariant is cheaper to state than to provision.** "No agent holds both a retrieval tool and a calculation tool" is one property of one file. Expressing it as a test makes it checkable in CI on every commit, where the project split was only checkable by deploying.
3. **The remaining controls are the ones that were already doing the work.** Authorization is re-applied in the tool body under the caller's identity, because a `function_call` carries none (ADR-019 rationale 2, unchanged). That is what stops a compromised turn acting *as someone else*, and it is unaffected by which project hosts the agent.

**Alternatives considered:** *Keep two projects* (rejected here, but it remains the stronger control on the single axis it covers; see Consequences). *One project, and rely on instructions telling readers not to call tools* (rejected outright: an instruction is not a control, and it is exactly what a prompt injection targets). *One project, with the readers moved to a separate Foundry account* (rejected: the same operational cost as two projects plus cross-account identity, for a boundary at the wrong granularity). *Rename one existing project* (not possible — Azure has no rename for a Foundry project; a renamed project is a new resource, which is why both prior projects are deleted rather than migrated).

**Consequences:** This is a net reduction in security posture on one axis, and it should be recorded as such rather than presented as a pure simplification. A reviewer can now add `simulate_energy_dispatch` to the procedure agent and the platform will not stop them; only `tests/knowledge/test_agent_manifest.py` will. Anyone holding data-plane access to the project can now enumerate and invoke every agent with one credential, where previously two were needed. The mitigation is that the invariant is asserted in CI against named agents, that the tool bodies remain the actual authorization point, and that the surface which carries tool access filters on tool ownership rather than trusting the caller's `agent` parameter.

Operationally the estate loses one project, one capability host, three connections and seven role assignments; the serialisation dependency between capability hosts disappears with the second host. Services read one endpoint variable, and `FOUNDRY_OPERATIONS_PROJECT_ENDPOINT` is removed everywhere. Because Foundry projects cannot be renamed, applying this decision to the running demo estate means deleting `novasteelv3-proj` and `novasteelv3-ops-proj` and losing their deployed agents and conversation history; the roster is recreated by `agent_reconciler.py`, which is the reason the reconciler exists. A Connected Agent topology is no longer blocked by project boundaries, which is a capability gained and a boundary that must now be argued rather than assumed if it is ever adopted.
...
│   └── device-simulator/             # Deterministic ring-buffer device/sensor simulator (17 devices, 91 sensors across 4 sites); in-process library + standalone FastAPI app
├── simulator/
