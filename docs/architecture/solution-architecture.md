...
The shell is a host, not a second business backend. The React bundle is versioned with the shell for the demonstration to avoid shell/MFE contract skew. The shell exposes only typed context/events: `themeMode`, `locale`, `activePersona`, `site`, `demoMode`, navigation intent, toast, capacity request, and telemetry. It does **not** hand a workload credential to React.
