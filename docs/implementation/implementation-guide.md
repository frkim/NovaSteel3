...
| `analytics-mfe` | TypeScript / React + MUI + D3 | KPI cards, D3 charts, `TBL-STD` virtualized/filterable tables (see `api-contracts.md` §5), optional Power BI embed, SSE alert consumption | Authentication, storing a service/API token beyond a host-brokered short-lived reference, direct Fabric/Foundry calls | `portal-shell` (interop), `bff-api` (HTTPS, token via host broker) |
...
| `knowledge-orchestrator` | Python | Consent/workflow state machine, Speech Fast Transcription request, draft-writing tool mediation, review-state tracking | Publishing an unreviewed procedure, bypassing the Knowledge Engineer approval gate | Azure Speech, Foundry Agent Service, restricted storage/search, `bff-api` |
...
| `FE-001` | `portal-shell` Blazor host: MSAL sign-in, chrome, routing, typed interop bridge | `FOUND-001`, `FE-000` | Shell exposes exactly the typed context named in `solution-architecture.md` §5.1 (`themeMode`, `locale`, `activePersona`, `site`, `demoMode`, navigation intent, toast, capacity request, telemetry); no workload credential crosses the bridge |
