...
@description('Create the Agent Service capability host. A capability host is IMMUTABLE: once created it cannot be repointed at different Search/Cosmos/Storage accounts, and changing them means recreating the project. Leave false until the three stores above are final.')
...
// One project holds the whole roster. It is named for the platform itself rather
// than carrying a `-proj` suffix, because there is no longer a second project to
// distinguish it from (ADR-020 supersedes ADR-019).
var projectName = resourcePrefix
...
resource project 'Microsoft.CognitiveServices/accounts/projects@2025-04-01-preview' = {
  ...
  properties: {
    ...
    description: 'Hosts the whole NovaSteel agent roster: the procedure agent and its Foundry IQ knowledge base, and the tool-calling operations agents whose results are proposals requiring human approval (ADR-006, ADR-007).'
  }
}
